...
def commands():
    ...
    #rate = rospy.Rate(50)
        while not False:
            pubmot1.publish(Float64(motor1))
            pubmot2.publish(Float64(motor2))
            pubmot3.publish(Float64(motor3))
            pubmot4.publish(Float64(motor4))
            pubmot5.publish(Float64(motor5))
            angles = rospy.wait_for_message('/robot/joint_states', JointState, timeout = 5)

	    if (abs(angles.position[4] - motor1) < 0.01 and abs(angles.position[1] - motor2) < 0.01 and \
	    abs(angles.position[2] - motor3) < 0.01 and abs(angles.position[3] - motor4) < 0.01 and abs(angles.position[0] - motor5) < 0.01):
                break
    #rate.sleep()
